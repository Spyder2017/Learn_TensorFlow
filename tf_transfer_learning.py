...
data_path = 'F:\\Users\\Flylee\\Documents\\tensorflow\\for_transfer_learning\\'

def download():	 # download tiger and kittycat image
	categories = ['tiger', 'kittycat']
	for category in categories:
		os.makedirs(data_path + 'data\\%s' % category, exist_ok=True)
		with open(data_path + 'imagenet_%s.txt' % category, 'r') as file:
			urls = file.readlines()
			n_urls = len(urls)
			for i, url in enumerate(urls):
				try:
					urlretrieve(url.strip(), data_path + 'data\\%s\\%s' % (category, url.strip().split('/')[-1]))
